...
NUM_INSTANCIAS = 20
VOTANTES_POR_INSTANCIA = 2000
EJECUCIONES_POR_CASO = 3
SEED_BASE = 20260726
...
def generar_heatmap(matriz, candidatos, estadisticas, instancia_id, output_dir):
    ...
    base = output_dir / f"heatmap_instancia_{instancia_id:02d}"
    posiciones_medias = {
...
def ejecutar_caso(ranking, instancia_id, total_instancias, num_votantes, configuracion, objetivo, objetivo_idx, ejecuciones, seed_instancia, output_dir):
    ...
    for ejecucion_idx in range(ejecuciones):
        ...
        graficos_dir = (
            output_dir
            / "graficos"
            / f"{objetivo['categoria']}_{candidato_objetivo}"
...
def guardar_resultados(resumenes, detalles, posiciones, output_dir):
    ...
    resumen_path = output_dir / "resultados_resumen.csv"
    detalles_path = output_dir / "resultados_ejecuciones.csv"
    posiciones_path = output_dir / "posiciones_medias.csv"
    categorias_path = output_dir / "resumen_categorias.csv"
