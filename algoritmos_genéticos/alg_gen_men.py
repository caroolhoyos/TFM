# Variante donde para si se obtiene la victoria en una ronda inferior al límite
...
def simular_ronda(Pi_r, ranking, candidatos):
    ...
    puntuaciones = dict.fromkeys(candidatos, 0)
...
def anular_rondas(child, desde):
    # Anula las rondas que ya no se simularán
    for ronda_restante in child[desde:]:
        ronda_restante[:] = [0] * len(ronda_restante)


def fitness(resultado, cand, max_rondas):
    # Función de fitness
    ganador, _, _, rondas_sobrevive, _, _ = resultado
...
def mutar_ronda(ronda, n_vivos, mutation_prob, sigma_efectiva, a, b):
    # Limita las posiciones activas por los candidatos vivos y la profundidad del ranking
    n_posiciones_activas = min(n_vivos, len(ronda))
    hubo_mutacion = False
    if mutation_prob > 0:
...
def simular_eleccion(Pi, ranking, cand, k, candidatos=None):
    # Simula las rondas hasta que el candidato sea eliminado o lidere en una de ellas
    ...
    if cand not in vivos:
        return None, vivos, False, rondas_sobrevive, ultima_puntuacion, ronda_ganadora
    ...
    for r in range(rondas_a_simular):
        if len(vivos) == 1:
            ganador = vivos[0]
            if ganador == cand and ronda_ganadora is None:
                ronda_ganadora = rondas_sobrevive
            return ganador, vivos, cand in vivos, rondas_sobrevive, ultima_puntuacion, ronda_ganadora

        Pi_r = Pi[r]
        eliminados, nuevos_vivos, puntuaciones = simular_ronda(Pi_r, ranking, vivos)
        ultima_puntuacion = puntuaciones
        lider_ronda = obtener_ganador(vivos, puntuaciones, resolver_por_puntuacion=True)
        if cand in eliminados:
            ganador = nuevos_vivos[0] if len(nuevos_vivos) == 1 else None
            return ganador, nuevos_vivos, False, rondas_sobrevive, ultima_puntuacion, ronda_ganadora
        ...
        if lider_ronda == cand and ronda_ganadora is None:
            # Considera ganador al candidato desde su primera ronda como líder
            ronda_ganadora = rondas_sobrevive
            return cand, vivos, cand in vivos, rondas_sobrevive, ultima_puntuacion, ronda_ganadora
...
def simular_eleccion_mutante(child, candidatos, cand, max_rondas, mutation_prob, sigma_efectiva, a, b, ranking):
    # Simula, muta y detiene la evaluación cuando el candidato sea ganador para una ronda dada
    vivos = candidatos[:]
    rondas_sobrevive = 0
    ultima_puntuacion = {}
    ronda_ganadora = None
    ...
    if cand not in vivos:
        anular_rondas(child, 0)
        return None, vivos, False, 0, ultima_puntuacion, ronda_ganadora
    ...
    if max_rondas == 0:
        ...
        if ganador == cand:
            ronda_ganadora = 0
        return ganador, vivos, True, 0, ultima_puntuacion, ronda_ganadora
    ...
    for r in range(rondas_a_simular):
        ...
        if lider_ronda == cand and ronda_ganadora is None:
            # Guarda la primera ronda ganadora y anula las rondas posteriores
            ronda_ganadora = rondas_sobrevive
            anular_rondas(child, rondas_sobrevive)
            return (cand, vivos, cand in vivos, rondas_sobrevive, ultima_puntuacion, ronda_ganadora,)
    ...
    anular_rondas(child, rondas_a_simular)
    return (ganador, vivos, cand in vivos, rondas_sobrevive, ultima_puntuacion, ronda_ganadora,)
...
def genetic_election(ranking, cand, generations, mutation_prob, pop_size, k=None, a=0, b=100, return_history=False, selection_method="rank"):
    ...
    # Inicializa el ranking y la población
    ...
    population, population_resultados = mutate(inicializar_P(pop_size, profundidad_ranking, k, a, b), 0, a, b, ranking, candidatos, cand, max_rondas,)
